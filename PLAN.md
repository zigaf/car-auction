...
- [ ] Нет cron/scheduled task для определения победителя аукциона (Рекомендуется BullMQ/Redis)
...
- [x] Модуль `auction`:
  ...
  - [ ] Определение победителя по завершении таймера (Delayed Jobs через BullMQ/Redis, не cron)
...
### Фаза 3.5 — Инфраструктура и UI Kit (NEW ⚡)

**API (Оптимизация и Надежность):**
- [ ] Настройка Redis (BullMQ для очередей + RedisAdapter для Socket.IO)
- [ ] Механизм `resolveAutoBids`: перенос рекурсивных вызовов в фоновые задачи (BullMQ), чтобы не блокировать БД
...
### Фаза 5 — Админ-панель (MVP) (ПЕРЕОПРЕДЕЛЕНО ИЗ ФАЗЫ 6)

**API & Client:**
- [ ] Базовое управление лотами (CRUD, изменение статусов)
- [ ] Базовая модерация документов (одобрение/отклонение)
- [ ] Управление балансом пользователей (для ручных корректировок)
- [ ] Просмотр всех заказов

*(Полноценная админка с ролями, статистикой, CMS и логами будет реализована в Фазе 7 или позже).*
